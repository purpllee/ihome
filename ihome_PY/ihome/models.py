...
class User(BaseModel, db.Model):
    ...
    houses = db.relationship("House", backref="user")  # 用户发布的房屋,cackref反引用。可以通过User.houses获取House中的数据。House.user也可以获取user中的数据
    orders = db.relationship("Order", backref="user")  # 用户下的订单


    @property     # 将函数转变为属性的装饰器,属性名就是函数名
    def password(self):
        """读取属性的逻辑行为"""
        # 函数的返回值会作为属性值
    ...
    # 需要先创建出读取属性，在如下设置，就可以设置属性了
    @password.setter
    def password(self, value):
        """设置属性的逻辑性为
        :params value   设置属性值的数据，value就是xxxx"""
    ...
    def to_dict(self):
        ...
        user_dict = {
            ...
            "name": self.name,
            "mobile":self.mobile,
            "avatar":constant.QINIU_URL_DOMAIN+ self.avatar_url if self.avatar_url else "",
...
class House(BaseModel, db.Model):
    ...
    orders = db.relationship("Order", backref="house")  # 房屋的订单. secondary表示二次查询
    ...
    def to_full_dict(self):
        ...
        for image in self.images:
            img_urls.append(constant.QINIU_URL_DOMAIN + image.url)
